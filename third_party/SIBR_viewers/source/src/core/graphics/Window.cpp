...
namespace sibr
{
	...
	{
		// Window positioning is intentionally unsupported by the Wayland
		// protocol. GLFW reports it as a platform error, but the window and
		// rendering context remain fully usable.
		if (description != nullptr &&
			std::string(description).find("window position") != std::string::npos) {
...
#else
		// On a Wayland desktop GLFW can create a valid native EGL context even
		// though the distro GLEW library reports that no GLX display exists.
		// OpenGL entry points are initialized in that case, so this is benign.
		if (err != GLEW_OK && err != GLEW_ERROR_NO_GLX_DISPLAY)
#endif
			SIBR_ERR << "cannot initialize GLEW (used to load OpenGL function): "
				<< glewGetErrorString(err) << " [" << err << "]" << std::endl;
