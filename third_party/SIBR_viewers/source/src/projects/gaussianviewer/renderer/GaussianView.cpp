...
void sibr::GaussianView::onGUI()
{
	...
	else if (currMode == "Ellipsoids")
	{
		ImGui::Checkbox("X-ray (show interior)", &_ellipsoidXray);
		ImGui::SliderInt("Display every Nth", &_ellipsoidStride, 1, 100);
		ImGui::Text("Displaying about %d / %d ellipsoids",
			(count + _ellipsoidStride - 1) / _ellipsoidStride, count);
		ImGui::SliderFloat("Ellipsoid Scale", &_ellipsoidScaleModifier, 0.01f, 2.0f);
		ImGui::SliderFloat("Opacity Min", &_ellipsoidAlphaMin, 0.0f, 1.0f);
		ImGui::SliderFloat("Opacity Max", &_ellipsoidAlphaMax, 0.0f, 1.0f);
		ImGui::SliderFloat("Opacity Strength", &_ellipsoidOpacityScale, 0.001f, 1.0f);
		if (!_ellipsoidXray)
